...
import handlebars0 from "handlebars";
import mongojs0 from "mongojs";
...
const app = express();
const handlebars = handlebars0;
const mongojs = mongojs0;
...
app
  ...
  .get("/authors/:author", function(req, res) {
    ...
    new Promise((res, req) => {
      collection.find({ author: author }).toArray((err, data) => {
        res({
          author,
          poems: data.map(el => {
